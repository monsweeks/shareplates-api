...
public interface OrganizationRepository extends JpaRepository<Organization, Long> {
    ...
    @Query("SELECT new com.giant.mindplates.biz.organization.vo.OrganizationStats(t.id, t.name, t.description, t.publicYn, t.creationDate, COUNT(u.id), (SELECT COUNT(t.id) FROM Topic t WHERE t.organization.id = t.id), (SELECT iou.role FROM OrganizationUser iou WHERE iou.user.id = :userId AND iou.organization.id = t.id)) " +
    ...
    List<OrganizationStats> findUserOrganizationStat(@Param("userId") Long userId, @Param("searchWord")  String searchWord, @Param("useYn") Boolean useYn, Sort sort);
